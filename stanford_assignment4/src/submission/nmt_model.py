...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### START CODE HERE (~ 8 Lines)
        # 1. Compute X by applying the source model embeddings
        X = self.model_embeddings.source(source_padded)

        # 2. Pack the padded sequence X
        packed_X = nn.utils.rnn.pack_padded_sequence(X, source_lengths)

        # Apply the encoder to the packed sequence
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(packed_X)

        # Unpack the padded sequence
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens_packed)

        # Permute enc_hiddens to have shape (b, src_len, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        # 3. Compute init_decoder_hidden and init_decoder_cell
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 2. Construct tensor `Y` of target sentences using target embeddings
        Y = self.model_embeddings.target(target_padded)

        # 3. Iterate over time steps
        for Y_t in torch.split(Y, 1):
            # Squeeze the time step dimension
            Y_t = torch.squeeze(Y_t, dim=0)  

            # Concatenate Y_t with the previous output
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)

            # Use the step function to compute the next decoder states and combined output
            dec_state, o_t,_ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)

            # Append the new combined output to the list
            combined_outputs.append(o_t)

            # Update o_prev to the new combined output
            o_prev = o_t

        # 4. Convert combined_outputs to a tensor
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. Apply softmax to e_t to yield attention distribution
        ...
        V_t = self.combined_output_projection(U_t)
         ...
         # 5. Compute O_t by first applying the Tanh function, then the dropout layer
